...
if ($query)
{
  ...
  try
  {
    ...
    if(strtolower($query)==strtolower($correct1)){
      ...
      $div =true;
      $results = $solr->search($query, 0, $limit, $additionalParameters);
      $link = "http://localhost/solr-php-client/hw5.php?q=$correct&sort=$choice";
...
<html>
  ...
  <body>
    ...
    <script>
   $(function() {
     var URL_PREFIX = "http://localhost:8983/solr/boston_globe/suggest?q=";
...
$csvArray =  array_map('str_getcsv', file('Boston Global Map.csv'));
$count =0;
$pre="";
// display results
if ($results)
...
<?php
  ...
  foreach ($results->response->docs as $doc)
  {
    ...
    $id = $doc->id;
    $or_id = $id;
    $id = str_replace("/usr/local/solr-7.1.0/server/solr/boston_globe/data/BG/","",$id);
    $descp = $doc->description;
    $title = $doc->title;
    ...
    $html_to_text_files_dir = "/usr/local/solr-7.1.0/server/solr/boston_globe/data/BG/";///////
